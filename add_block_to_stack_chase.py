...
class AddBlockToStackChaseCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit, sc_region, insert_at = None):
        ...
        inserted_char_cnt = self.view.insert(edit, insert_at, sc_region.get_text())
        sc_region.foreign_region = sublime.Region(insert_at, insert_at + inserted_char_cnt)
    ...
    def format_region(self, edit, sc_region):
        ...
        self.view.add_phantom(line_key, og_region, content, sublime.LAYOUT_INLINE)
        # Offset the region by the number of characters we added in the step above.
        region = sublime.Region(og_region.begin() + inserted_char_cnt, og_region.end() + inserted_char_cnt)
        ...
        for i, line in enumerate(self.view.split_by_newlines(region)):
            ...
            content = GUTTER_TEXT_STYLE + '<span>' + line_num_fmt.format(line_num) + '</span>'
            self.view.add_phantom(line_key, line, content, sublime.LAYOUT_INLINE)
        # Add one line of space after the last line of the block.
        line_key = key + 'after'
        content = '<br>'
        after_region = sublime.Region(og_region.end(), og_region.end())
        self.view.add_phantom(line_key, after_region, content, sublime.LAYOUT_BLOCK)
    ...
    def get_line_number_format(self):
        ...
        # TODO: Make the space left-padding work.
